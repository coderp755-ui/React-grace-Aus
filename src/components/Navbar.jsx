    ...
    <nav style={{
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px",
        ...
        <Link to="/" style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", textDecoration: "none" }}>
          ...
          <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700,
            fontSize: 18, color: "#0A1F44", letterSpacing: "-0.3px" }}>
          ...
          {NAV_LINKS.map(l => (
            <Link key={l.label} to={l.path}
              style={{ background: "none", border: "none", cursor: "pointer",
                ...
                color: scrolled ? "#2D3748" : "#fff", transition: "all 0.2s",
                textDecoration: "none", display: "block",
              }}
              onMouseEnter={e => { e.target.style.background = scrolled ? "#F0F4FF" : "rgba(255,255,255,0.15)"; e.target.style.color = scrolled ? "#1565C0" : "#fff"; }}
              onMouseLeave={e => { e.target.style.background = "none"; e.target.style.color = scrolled ? "#2D3748" : "#fff"; }}>
              {l.label}
            </Link>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => setOpen(!open)} style={{
            ...
            color: scrolled ? "#0A1F44" : "#fff", padding: 4,
          }}>
      ...
      {open && (
        <div style={{
          ...
          {NAV_LINKS.map(l => (
            <Link key={l.label} to={l.path} onClick={handleNavClick} style={{
              background: "none", border: "none", cursor: "pointer",
              textAlign: "left", padding: "12px 8px", borderRadius: 8,
              fontFamily: "'DM Sans', sans-serif", fontWeight: 500, fontSize: 15,
              color: "#2D3748", borderBottom: "1px solid #F0F4FF",
              textDecoration: "none", display: "block",
            }}>
              {l.label}
            </Link>
